...
def main(argv):
    ...
    send_request(sock, json.dumps({"username":username, "moviename":moviename, "starttime":starttime}))
    response = get_response(sock)
    dump_response()

# sends text over udp using global udp info
def send_request(sock, json_message):
    print_message("Request message: " + json_message)
    print_message("Sending request to server...")

    sock.sendto(json_message, ("10.10.1.1", 5000))
